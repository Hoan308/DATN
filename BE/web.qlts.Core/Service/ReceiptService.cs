...
using web.qlts.Application.Enum;
...
namespace web.qlts.Application.Service
{
    ...
    {
        private readonly Domain.Interface.Repository.IReceiptRepository _receiptRepository;
        private readonly IReceiptManager _receiptManager;


        public ReceiptService(Domain.Interface.Repository.IReceiptRepository receiptRepository, IReceiptManager receiptManager, IMapper mapper, IUnitOfWork unitOfWork) : base(receiptRepository, mapper, unitOfWork)
        {
        ...
        public async Task<object> CheckReference(Guid receiptId)
        {
            ...
            var listAssetByReceipAccuredId = new List<Domain.Entity.Asset>();
        ...
        private void CalculateDepreciationAndResidual(IEnumerable<AssetTranferDto> assetTranferDtos)
        {
            foreach (var asset in assetTranferDtos)
            {
                ...
                //Hao mòn lũy kế 
                asset.DepreciationAmount = (decimal)((float)asset.DepreciationYear * (DateTime.Now.Year - asset.TrackedYear));

                //Giá trị còn lại
                asset.ResidualPrice = (decimal)(asset.Quantity * (float)asset.Cost - (float)asset.DepreciationAmount);
            }
        }
        ...
        public async Task<object> GetPagging(int pageSize, int pageNumber, string? searchInput)
        {
            ...
            {
                toltalResidualPrice += receipt.ResidualPrice;
                totalCost += receipt.OrgPrice;
            }
